...
from video_chatgpt.video_conversation import load_video, mod_load_video
...
class Chat:
    ...
    def upload_video(self, video, img_list, mod = 0):
        if isinstance(video, str):  # is a path
            frames = load_video(video)
            if mod != 0:
                frames = mod_load_video(video, load_mod = mod)
    ...
    # def upload_curve_video(self, video, img_list):
    #     if isinstance(video, str):  # is a path
    #         frames = mod_load_video(video,load_mod=1)
    #         image_tensor = self.image_processor.preprocess(frames, return_tensors='pt')['pixel_values']
    #         img_list.append(image_tensor)
    #     else:
    #         raise NotImplementedError
    ...
    def answer(self, state, img_list, temperature, max_new_tokens, first_run, mist_feature=None):
        if state.skip_next:
        ...
        if mist_feature is not None:
            mist_feature = torch.tensor(mist_feature).unsqueeze(0)
            mist_feature = mist_feature.repeat(1, 2).float().half().cuda()
            video_spatio_temporal_features = torch.cat((video_spatio_temporal_features, mist_feature), dim = 0)
        print("video_features shape is {}".format(video_spatio_temporal_features.shape))
        with torch.inference_mode():
            output_ids = self.model.generate(
                ...
                do_sample=True,
                temperature=float(temperature),
                max_new_tokens=min(int(max_new_tokens), 1536),    # ori: 1536
