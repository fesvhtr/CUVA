...
def mod_load_video(vis_path, n_clips=1, num_frm=100, add_frm=200, load_mod=None):
    ...
    if (load_mod == 1):  # 固定帧数算法
        ...
        pop_list = []
        pop_list = random.sample(mod_lst, len(add_list))
        pop_list.sort()
...
def initialize_model(model_name, projection_path=None, vision_tower_path=None):
    """
    ...
    image_processor = CLIPImageProcessor.from_pretrained(vision_tower_path, torch_dtype=torch.float16)
    ...
    vision_tower = CLIPVisionModel.from_pretrained(vision_tower_path, torch_dtype=torch.float16,
