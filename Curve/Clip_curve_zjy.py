...
def generate_frames(video_src, frame_interval=30, peak_time_interval=1, dense_sample=10, coarse_sample=5, times=0, peak_list=None):  # 帧间隔默认30
    ...
    while rval:
        ...
        if times == 1:
            ...
            for peak in peak_list:
                samples = np.linspace((peak+1)*30-peak_time_interval*30, (peak+1)*30+peak_time_interval*30, dense_sample*2)  # todo:要注意开投帧和结尾帧的越界问题
...
#     # openai.organization = "org-LsauuKtncHakIhdjOdmqRLOk"  # 设置区域码，在OpenAI账号中查看
...
def cal_text_emb(text_list):
    text = clip.tokenize(text_list).to(device)  # 将这三句话向量化
    score = torch.tensor([0.5, 0.4, 0.3, 0.2]).to(device)  # todo: default: [0.5, 0.4, 0.3, 0.2, 0.1]
    text_emb = model.encode_text(text)
    return text_emb, score


def cal_sim(img_emb, text_emb, score):
    # img_emb /= img_emb.norm(dim=-1, keepdim=True)    # 归一化
    # text_emb /= text_emb.norm(dim=-1, keepdim=True)
    similarity = (100.0 * img_emb @ text_emb.T).softmax(dim=-1).T
    similarity /= similarity.norm(dim=-1, keepdim=True)  # shape: [5, 36]
    score = score.unsqueeze(1)   # shape: [5, 1]
...
if __name__ == "__main__":
    ...
    peak_list = [4, 7, 15]
    generate_frames(src, peak_list=peak_list, times=1)
