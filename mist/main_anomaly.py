...
from train.train_anomaly import train, eval, infer
...
if __name__ == "__main__":
    ...
    if args.test:
        ...
        logging.info("number of infer instances: {}".format(len(infer_loader.dataset)))


    # Loss + Optimizer
    criterion = nn.CrossEntropyLoss()

    params_for_optimization = list(p for n, p in model.named_parameters() if p.requires_grad and n.split('.')[1] == 'vqproj')
    ...
    h5pyfile = "/home/disk1/anomaly_mist/mist_features"
    num_videos, h5pyfile = infer(model, infer_loader, args, h5pyfile)
    logging.info("{} videos features extracted done in {}".format(num_videos, h5pyfile))
