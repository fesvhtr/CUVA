...
class EncoderVid(nn.Module):
    ...
    def forward(self, video_o):
        bsize, numc, numf, numr, fdim =  video_o.shape  # bs, 8, 4, 16, 517
       
        video_o = video_o.view(bsize, numc*numf, numr, fdim)  # bs, 32, 16, 517
        roi_feat = video_o[:,:,:, :self.dim_feat]
        roi_bbox = video_o[:,:,:, self.dim_feat:(self.dim_feat+self.dim_bbox)]   # bs, 32, 16, 5
