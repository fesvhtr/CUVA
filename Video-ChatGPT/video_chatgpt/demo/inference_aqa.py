...
def cmd_upload_image(chat, video_path, state):
...
def test_AIST(chat, video_path, question, mist_feature=None):
    state = None
    img_list = []
    temperature = args.temp
    max_output_tokens = args.max_tokens
    state, img_list, first_run = cmd_upload_image(chat, video_path, state)
    state, moderation_msg = cmd_add_text(state, question, video_path, first_run)
...
if __name__ == "__main__":
    ...
    for i in tqdm(output_data):
        ...
        try:
            ...
            ans = test_AIST(chat, os.path.join(video_dir, video), prompt, mist_feature)
            i['output'] = ans
