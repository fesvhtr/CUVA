...
def append_msg(state, text, image, first_run):
...
def upload_image(chat, video_path, state):
    ...
    img_list = []
    first_run = True
    img_list = chat.upload_video(video_path, img_list)
...
def test_single_video(chat, video_path, question):
    ...
    state, moderation_msg = append_msg(state, question, video_path, first_run)
...
def inference(chat, video_path, question, mist_feature=None):
    state = None
    img_list = []
    temperature = args.temp
    max_output_tokens = args.max_tokens
    state, img_list, first_run = upload_image(chat, video_path, state)
    state, moderation_msg = append_msg(state, question, video_path, first_run)
...
if __name__ == "__main__":
    ...
    conv_mode = "video-chatgpt-score"
    # Create chat for the demo
    chat = Chat(args.model_name, conv_mode, tokenizer, image_processor, vision_tower, model, replace_token)
    ...
    for i in tqdm(output_data):
        ...
        try:
            ...
            ans = inference(chat, os.path.join(video_dir, video), prompt)
            i['output'] = ans
